...
Bu API, Visual Studio, NuGet. exe ve .NET CLı içindeki NuGet istemcisi tarafından [`dotnet restore`](/dotnet/core/tools/dotnet-restore?tabs=netcore2x), Visual Studio Kullanıcı arabiriminde arama ve [`nuget.exe push`](../reference/cli-reference/cli-ref-push.md)gibi NuGet işlemlerini gerçekleştirmek için kullanılır.
...
Hizmet dizini şema sürümü, hizmet dizinindeki `version` özelliği tarafından belirtilir. Sürüm dizesinin ana sürüm numarası `3`sahip olduğu API. Hizmet dizini şemasında, önemli olmayan değişiklikler yapıldığından, sürüm dizesinin ikincil sürümü artar.
...
> [!Note]
> Bir kaynak uygulama `SearchAutocompleteService`, otomatik tamamlama davranışı düzgün şekilde devre dışı bırakılmalıdır. `ReportAbuseUriTemplate` uygulanmadığından, resmi NuGet istemcisi NuGet. org 'ın uygunsuz kullanım URL 'sine geri döner ( [NuGet/Home # 4924](https://github.com/NuGet/Home/issues/4924)tarafından izlenir). Diğer istemciler, kullanıcıya bir rapor kötüye kullanımı URL 'SI göstermeyebilir.
...
BAŞLı   | Karşılık gelen `GET` isteği için yanıt üst bilgilerini getirir.
...
Bir API uç noktasına yapılan tüm `GET` istekleri HTTP yeniden yönlendirme (301 veya 302) döndürebilir. İstemciler `Location` üst bilgisini gözlemleyerek ve sonraki bir `GET`yayımlayarak bu yeniden yönlendirmeleri sorunsuz bir şekilde işlemelidir. Belirli uç noktalarla ilgili belgeler, yeniden yönlendirmelerin nerede kullanılabileceğini açıkça çağırmaz.
...
X-NuGet-Client-Version   | **Kullanım dışı** ve `X-NuGet-Protocol-Version` tarafından değiştirilmiş
...
X-NuGet-Session-Id       | *İsteğe bağlı*. NuGet istemcileri v 4.7 + aynı NuGet istemci oturumunun parçası olan HTTP isteklerini belirler.

`X-NuGet-Session-Id`, `PackageReference`tek bir geri yükleme ile ilgili tüm işlemler için tek bir değere sahiptir. Otomatik tamamlama ve `packages.config` geri yükleme gibi diğer senaryolar için kodun birbirine nasıl bağlı olduğu ile ilgili birkaç farklı oturum KIMLIĞI olabilir.

## <a name="authentication"></a>Kimlik Doğrulama

Kimlik doğrulaması, tanımlanacak paket kaynak uygulamasına bırakılır. Nuget.org için yalnızca `PackagePublish` kaynağı özel bir API anahtarı üst bilgisi aracılığıyla kimlik doğrulaması gerektirir. Ayrıntılar için bkz. [`PackagePublish` kaynağı](package-publish-resource.md) .
