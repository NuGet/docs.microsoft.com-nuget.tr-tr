...
`.nuspec` dosyası, paket meta verilerini içeren bir XML bildirimidir. Bu bildirim her ikisi de paketini derlemek ve tüketicilere bilgi sağlamak için kullanılır. Bildirim her zaman bir pakete dahildir.
...
- `packages.config`kullanan SDK olmayan projeler için `nuget.exe pack` `.nuspec` kullanın.
...
- [SDK stilindeki projelere](../resources/check-project-format.md) yönelik paketler oluşturmak için bir `.nuspec` dosyası gerekli değildir (genellikle .NET Core ve [sdk özniteliğini](/dotnet/core/tools/csproj#additions)kullanan .NET Standard projeler). (Paketi oluştururken bir `.nuspec` oluşturulduğunu unutmayın.)
   ...
   `dotnet.exe pack` veya `msbuild pack target`kullanarak bir paket oluşturuyorsanız, genellikle proje dosyasındaki `.nuspec` dosyasında bulunan [tüm özellikleri dahil](../reference/msbuild-targets.md#pack-target) etmenizi öneririz. Ancak, bunun yerine [`dotnet.exe` veya `msbuild pack target`kullanarak paketiçin bir `.nuspec` dosyası kullanmayı ](../reference/msbuild-targets.md#packing-using-a-nuspec)tercih edebilirsiniz.

- `packages.config`, [Packagereference](../consume-packages/package-references-in-project-files.md)'a geçirilen projeler için, paketi oluşturmak için bir `.nuspec` dosyası gerekli değildir. Bunun yerine, [MSBuild-t:Pack](../consume-packages/migrate-packages-config-to-package-reference.md#create-a-package-after-migration)kullanın.
...
Nuget.org üzerindeki profil adlarını kullanan paket oluşturucularının virgülle ayrılmış listesi. Bu genellikle `authors`ile aynıdır ve paket nuget.org 'e yüklenirken yok sayılır. Bkz. [NuGet.org üzerinde paket sahiplerini yönetme](../nuget-org/publish-a-package.md#managing-package-owners-on-nugetorg). 
...
> [!Tip]
> `icon`desteklemeyen kaynaklarla geriye dönük uyumluluğu sürdürmek için hem `icon` hem de `iconUrl` belirtebilirsiniz. Visual Studio, gelecekteki sürümlerde bulunan klasör tabanlı bir kaynaktan gelen paketlere yönelik `icon` destekleyecektir.
...
UI görüntülemesi için paketin kısa bir açıklaması. Atlanırsa, `description` kesilen bir sürümü kullanılır.
...
*(3,5 +)* Geleneksel bir bağımlılık paketi dışında paketin türünü belirten sıfır veya daha fazla `<packageType>` öğesi koleksiyonu. Her packageType 'ın *ad* ve *Sürüm*öznitelikleri vardır. Bkz. [paket türünü ayarlama](../create-packages/set-package-type.md).
#### <a name="dependencies"></a>bağımlılıklar
Paketin bağımlılıklarını belirten sıfır veya daha fazla `<dependency>` öğesi koleksiyonu. Her bağımlılığın *kimliği*, *sürümü*, *içerme* (3. x +) ve *exclude* (3. x +) öznitelikleri vardır. Aşağıdaki [bağımlılıklara](#dependencies-element) bakın.
#### <a name="frameworkassemblies"></a>frameworkAssemblies
*(1.2 +)* Bu paketin gerektirdiği .NET Framework derleme başvurularını tanımlayan sıfır veya daha fazla `<frameworkAssembly>` öğelerinden oluşan bir koleksiyon. Bu, başvuruların paketi kullanan projelere eklenmesini sağlar. Her frameworkAssembly *AssemblyName* ve *TargetFramework* öznitelikleri vardır. Aşağıdaki [Framework derleme BAŞVURULARı GAC 'Yi belirtme](#specifying-framework-assembly-references-gac) bölümüne bakın.
#### <a name="references"></a>başvurular
*(1,5 +)* Paket, proje başvuruları olarak eklenen `lib` klasöründeki derleme `<reference>` bir sıfır veya daha fazla öğe koleksiyonu. Her başvurunun bir *Dosya* özniteliği vardır. `<references>`, *TargetFramework* özniteliğiyle birlikte `<reference>` öğeleri içeren bir `<group>` öğesi de içerebilir. Atlanırsa, `lib` tüm başvurular dahil edilir. Aşağıda [Açık derleme başvurularını belirtme](#specifying-explicit-assembly-references) bölümüne bakın.
#### <a name="contentfiles"></a>contentFiles
*(3.3 +)* Tüketim projesinde içerilecek içerik dosyalarını tanımlayan `<files>` öğelerinin koleksiyonu. Bu dosyalar, proje sistemi içinde nasıl kullanılması gerektiğini betimleyen bir öznitelikler kümesiyle belirtilmiştir. Aşağıdaki [pakete dahil edilecek dosyaları belirtme](#specifying-files-to-include-in-the-package) bölümüne bakın.
#### <a name="files"></a>files 
`<package>` düğümü `<metadata>`eşdüzey olarak bir `<files>` düğümü ve paketine hangi derleme ve içerik dosyalarının ekleneceğini belirtmek için `<metadata>`altındaki `<contentFiles>` alt öğesi içerebilir. Ayrıntılar için bu konunun ilerleyen kısımlarında [derleme dosyalarını](#including-assembly-files) ve [içerik dosyalarını](#including-content-files) dahil etme bölümüne bakın.

### <a name="metadata-attributes"></a>meta veri öznitelikleri

#### <a name="minclientversion"></a>MinClientVersion
NuGet. exe ve Visual Studio Paket Yöneticisi tarafından zorlanan, bu paketi yükleyesağlayan NuGet istemcisinin en düşük sürümünü belirtir. Bu, paket, NuGet istemcisinin belirli bir sürümünde eklenen `.nuspec` dosyanın belirli özelliklerine bağlı olduğunda kullanılır. Örneğin, `developmentDependency` özniteliği kullanan bir paket `minClientVersion`için "2,8" belirtmelidir. Benzer şekilde, `contentFiles` öğesini kullanan bir paket (sonraki bölüme bakın) `minClientVersion` "3,3" olarak ayarlanmalıdır. Ayrıca, 2,5 ' den önceki NuGet istemcileri bu bayrağı tanımadığı için, `minClientVersion` neleri içermesi gerektiğine bakılmaksızın paketi yüklemeyi *her zaman* reddeder.
...
Bir paket oluştururken, [`nuget pack` komutu](../reference/cli-reference/cli-ref-pack.md) , `.nuspec` dosyanın `<metadata>` düğümündeki $-Delimited belirteçlerini bir proje dosyasından ya da `pack` komutunun `-properties` anahtarından değiştirir.

Komut satırında, `nuget pack -properties <name>=<value>;<name>=<value>`belirteç değerleri belirtirsiniz. Örneğin, `.nuspec` `$owners$` ve `$desc$` gibi bir belirteç kullanabilir ve değerleri paketleme sırasında aşağıdaki gibi sağlayabilirsiniz:
...
Bir projeden değerleri kullanmak için, aşağıdaki tabloda açıklanan belirteçleri belirtin (AssemblyInfo, `AssemblyInfo.cs` veya `AssemblyInfo.vb`gibi `Properties` dosya anlamına gelir).

Bu belirteçleri kullanmak için `nuget pack`, yalnızca `.nuspec`yerine proje dosyası ile çalıştırın. Örneğin, aşağıdaki komutu kullanırken, bir `.nuspec` dosyasındaki `$id$` ve `$version$` belirteçleri projenin `AssemblyName` ve `AssemblyVersion` değerleriyle değiştirilmiştir:

```ps
nuget pack MyProject.csproj
```

Genellikle, bir projeniz olduğunda, bu standart belirteçlerden bazılarını otomatik olarak içeren `nuget spec MyProject.csproj` kullanarak başlangıçta `.nuspec` oluşturursunuz. Ancak, bir proje gerekli `.nuspec` öğeleri için değerler eksikse `nuget pack` başarısız olur. Ayrıca, proje değerlerini değiştirirseniz, paketi oluşturmadan önce yeniden oluşturmayı unutmayın; Bu, paket komutunun `build` anahtarıyla kolayca yapılabilir.

`$configuration$`dışında, projedeki değerler komut satırında aynı belirtece atanmış herhangi bir tercih halinde kullanılır.
...
`AssemblyName`, MSBuild 'teki `Release` yapılandırması ile `LoggingLibrary` olan bir derleme oluşturduğunuzda, paketteki `.nuspec` dosyasındaki sonuç çizgileri aşağıdaki gibidir:
...
`<metadata>` içindeki `<dependencies>` öğesi, üst düzey paketin bağımlı olduğu diğer paketleri tanımlayan herhangi bir sayıda `<dependency>` öğesi içerir. Her bir `<dependency>` için öznitelikler aşağıdaki gibidir:
...
| exclude | Son pakette hariç tutulacak bağımlılığı belirten, etiketleri dahil et/hariç tut (aşağıya bakın) listesi. Varsayılan değer, üzerine yazılabilir `build,analyzers`. Ancak `content/ ContentFiles`, aşırı yazılabilir olmayan son pakette da örtük olarak dışlanır. `exclude` belirtilen Etiketler `include`belirtilen değerlere göre önceliklidir. Örneğin, `include="runtime, compile" exclude="compile"` `include="runtime"`aynıdır. |
...
Örneğin, aşağıdaki satırlar `PackageA` sürüm 1.1.0 veya üzeri bağımlılıkları ve sürüm 1. x ' i `PackageB` gösterir.
...
Aşağıdaki satırlar aynı paketlere yönelik bağımlılıkları gösterir, ancak `PackageA` ve `build` klasörlerini `contentFiles`, ancak `compile` `native` ve `PackageB`klasörlerini dahil etmek için belirtin.
...
> [!Important]
> `nuget spec`kullanarak bir projeden `.nuspec` oluştururken, söz konusu projede var olan bağımlılıklar, elde edilen `.nuspec` dosyasına otomatik olarak eklenmez. Bunun yerine, `nuget pack myproject.csproj`kullanın ve oluşturulan *. nupkg* dosyasının içinden *. nuspec* dosyasını alın. Bu *. nuspec* , bağımlılıkları içerir.

### <a name="dependency-groups"></a>Bağımlılık grupları

*Sürüm 2.0 +*

Tek bir düz listeye alternatif olarak, bağımlılıklar, `<dependencies>`içindeki `<group>` öğeleri kullanılarak hedef projenin çerçeve profiline göre belirtilebilir.
...
`targetFramework` özniteliği olmayan `<group>` öğesi, bağımlılıkların varsayılan veya geri dönüş listesi olarak kullanılır. Tam çerçeve tanımlayıcıları için bkz. [hedef çerçeveler](../reference/target-frameworks.md) .

> [!Important]
> Grup biçimi düz bir liste ile birlikte karıştırılamaz.

> [!Note]
> `lib/ref` klasöründe kullanılan [hedef çerçeve bilinen adı (tfd)](../reference/target-frameworks.md) biçimi, `dependency groups`kullanılan tfd ile karşılaştırıldığında farklıdır. `dependencies group` olarak belirtilen hedef çerçeveler ve `.nuspec` dosyanın `lib/ref` klasörü tam eşleşmeler yoksa, `pack` komut [NuGet WARNING NU5128](../reference/errors-and-warnings/nu5128.md)öğesini yükseltir.
...
Tek bir düz listeye alternatif olarak, başvurular, `<references>`içindeki `<group>` öğeleri kullanılarak hedef projenin çerçeve profiline göre belirtilebilir.
...
`targetFramework` özniteliği olmayan `<group>` öğesi, varsayılan veya geri dönüş listesi olarak kullanılır. Tam çerçeve tanımlayıcıları için bkz. [hedef çerçeveler](../reference/target-frameworks.md) .
...
Framework derlemeleri, .NET Framework 'ün bir parçası olan ve belirli bir makine için genel derleme önbelleğinde (GAC) olması gereken olanlardır. `<frameworkAssemblies>` öğesi içindeki bu derlemeler tanımlayarak, bir paket gerekli başvuruların projenin bu tür başvurularına sahip olmadığı olayda bir projeye eklendiğinden emin olabilir. Kuşkusuz bu tür derlemeler doğrudan bir pakete dahil edilmez.

`<frameworkAssemblies>` öğesi, her biri aşağıdaki öznitelikleri belirten sıfır veya daha fazla `<frameworkAssembly>` öğesi içeriyor:
...
Aşağıdaki örnek, tüm hedef çerçeveler için `System.Net` bir başvuru ve yalnızca .NET Framework 4,0 için `System.ServiceModel` başvurusu gösterir:
...
[Paket oluşturma](../create-packages/creating-a-package.md)bölümünde açıklanan kuralları izlerseniz, `.nuspec` dosyadaki bir dosya listesini açıkça belirtmeniz gerekmez. `nuget pack` komutu otomatik olarak gerekli dosyaları seçer.
...
| Öznitelik | Açıklama |
| --- | --- |
| **YN** | `exclude` özniteliği tarafından belirtilen Dışlamalar ile ilgili olarak, içerilecek dosyanın veya dosyaların konumu. Mutlak bir yol belirtilmediği takdirde yol `.nuspec` dosyasına görelidir. Joker karakter `*` izin verilir ve çift joker karakter `**` özyinelemeli bir klasör aramasını gerektirir. |
...
| **amaz** | `src` konumundan dışlanacak dosya veya dosya desenlerinin noktalı virgülle ayrılmış listesi. Joker karakter `*` izin verilir ve çift joker karakter `**` özyinelemeli bir klasör aramasını gerektirir. |
...
İçerik dosyaları, `target` özniteliğinde `content` klasörü belirtilerek `<files>` öğesi kullanılarak bir pakete dahil edilmiştir. Ancak, bu tür dosyalar, paket, `<contentFiles>` öğesini kullanan PackageReference kullanarak bir projeye yüklendiğinde yok sayılır.
...
İçerik dosyaları için yalnızca derleme dosyaları için aynı biçimi kullanın, ancak aşağıdaki örneklerde gösterildiği gibi `target` özniteliğinde temel klasör olarak `content` belirtin.
...
Bu durumda, NuGet `target` içindeki uzantının `src` uzantısıyla eşleşip eşleşmediği ve bu şekilde `target` adının bu bölümünü klasör olarak değerlendirmiş olduğunu görür:
...
Varsayılan olarak, bir paket içeriği `contentFiles` klasöre koyar (aşağıya bakın) ve varsayılan öznitelikleri kullanarak bu klasördeki tüm dosyaları dahil `nuget pack`. Bu durumda, `.nuspec` `contentFiles` bir düğüm eklemek gerekli değildir.

Hangi dosyaların ekleneceğini denetlemek için, `<contentFiles>` öğesi, tam dosyaları içeren bir `<files>` öğeleri koleksiyonudur.

Bu dosyalar, proje sistemi içinde nasıl kullanılması gerektiğini betimleyen bir öznitelikler kümesiyle belirtilir:

| Öznitelik | Açıklama |
| --- | --- |
| **include** | Istenir `exclude` özniteliği tarafından belirtilen Dışlamalar ile ilgili olarak, içerilecek dosyanın veya dosyaların konumu. Mutlak bir yol belirtilmediği takdirde yol `contentFiles` klasörüne görelidir. Joker karakter `*` izin verilir ve çift joker karakter `**` özyinelemeli bir klasör aramasını gerektirir. |
| **amaz** | `src` konumundan dışlanacak dosya veya dosya desenlerinin noktalı virgülle ayrılmış listesi. Joker karakter `*` izin verilir ve çift joker karakter `**` özyinelemeli bir klasör aramasını gerektirir. |
| **buildAction** | `Content`, `None`, `Embedded Resource`, `Compile`vb. gibi MSBuild için içerik öğesine atanacak yapı eylemi. Varsayılan değer `Compile`. |
...
NuGet, bir paket yüklerken `<contentFiles>` alt öğelerini yukarıdan aşağıya uygular. Aynı dosyayla birden çok giriş eşleşiyorsa, tüm girişler uygulanır. Aynı öznitelik için bir çakışma varsa en üstteki girdi alt girişleri geçersiz kılar.
...
- `codeLanguages`, belirli bir `any``cs`, `vb`, `fs`, `$(ProjectLanguage)` veya küçük harfli eşdeğeri olabilir
...
Boş klasörler, belirli dil birleşimleri ve TxM için içerik sağlamayı devre dışı bırakmak için `.` kullanabilir, örneğin:
...
Her `<group>` öğesi `targetFramework` bir öznitelik ve sıfır veya daha fazla `<frameworkReference>` öğesi gerektirir.
...
**Bağımlılıklar içeren bir `.nuspec`**
