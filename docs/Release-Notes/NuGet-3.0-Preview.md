...
NuGet 3.0 Önizleme bitleri paket kaynağı açılır listede "preview.nuget.org" adlı yeni bir paket kaynağı görürsünüz. Bu paket kaynağını seçerseniz, yeni API'mize yerine nuget.org için bağlanmak için kullanacağız. Önizleme kaynak kullanıcı arabiriminde kullanılabilir yaptık, biz sınamak için gözden geçirebilir ve yeni API geliştirmeye devam ederken. NuGet 3.0 RC'de bu yeni API v3 tabanlı paket kaynağı v2 tabanlı "nuget.org" Paket kaynağı yerini alır.
...
Şimdi arasında 3.0 RTM, ayrıca kullanıcı Arabiriminde ne göreceğiniz ötesinde bazı temel yeni NuGet özellikleri, çalışıyoruz. Belirgin yatırım alanlarının kısa bir listesi aşağıda verilmiştir:
