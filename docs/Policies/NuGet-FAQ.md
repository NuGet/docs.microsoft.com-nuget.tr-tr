...
Ayarlama [ `repositoryPath` ](../Schema/nuget-config-file.md#config-section) ayarı `Nuget.Config` kullanarak `nuget config -set repositoryPath=<path>`.
...
Evet. Üzerinde paketler için kimlikleri ayırabilirsiniz [nuget.org](https://www.nuget.org/) hesabınız için bir paket kimliği önek isteyen tarafından. Bir paket kimliği öneki istemek için (saat) paketi sahibi görünen adı ve istenen paket kimliği öneki ile nuget.org hesabı posta gönderin.  
